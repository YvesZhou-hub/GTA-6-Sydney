...
**v0.2.1-preview.1 · 持续增援、自动武器与战地快修**

[下载 Windows x86_64 试玩包](https://github.com/YvesZhou-hub/GTA-6-Sydney/releases/download/v0.2.1-preview.1/Harbourlife-Windows-x86_64.zip) · [下载 Mac 试玩包](https://github.com/YvesZhou-hub/GTA-6-Sydney/releases/download/v0.2.1-preview.1/Harbourlife-macOS-arm64.zip) · [所有发布](https://github.com/YvesZhou-hub/GTA-6-Sydney/releases)
...
选择“奶龙危机 · 开始生存”：奶龙会从附近可行走的地面自动出现并追击，开局 12 秒保护期间就会接近。左键或 X 反击，Tab 免费进入武装载具；副武器默认自动锁定和发射，V 随时开关。换街区不必清完旧敌人；清理进度决定奖金、敌人等级与附近增援数量，最高 Lv.10。
...
沿用 v0.2.0 的存档格式 6，可读取旧存档并保留金币、载具和导航；格式 6 存档不能用 v0.1.9 或更早的游戏读取，请保留更新前的存档备份。
...
本版沿用 v0.2.0 的存档格式 6，仍可读取旧格式 1–5，保存独立载具、导航、城市体验和生存进度。较大的新船艇会检查旧泊位净空，必要时调整静止旧副本；原有 ID、油量和损伤保留，空中姿态与安全桥下停车不会被搬到另一层道路。旧世界首次进入新地图时，会检查人物和已有载具是否被新建筑包住，只调整发生冲突的副本，保留 ID、油量、损伤与占用关系；之后正常保存记录地图版本。详见 [旧地图存档迁移](docs/SAVE_MAP_MIGRATION.md)。格式 6 存档须继续使用 v0.2.0 或兼容的新版应用；更旧版本会拒绝读取。仅载入旧存档不会重写原文件；首次成功保存时升级格式并保留之前的恢复副本。
