...
func run() -> void:
	...
	check("upgrade changes damage radius and cooldown without mutating base profile",profile.damage==157.5 and is_equal_approx(profile.radius,25.6) and is_equal_approx(profile.cooldown,.805) and Weapons.PROFILES.tank.damage==90.0 and Weapons.PROFILES.tank.radius==16.0 and Weapons.PROFILES.tank.cooldown==1.15)
	...
	check("layer 16 target produces typed gameplay blast independently",hits.size()==1 and hits[0].owner==tank and hits[0].damage==157.5 and is_equal_approx(hits[0].radius,25.6) and game.building_blasts==1,{"hits":hits.size()})
