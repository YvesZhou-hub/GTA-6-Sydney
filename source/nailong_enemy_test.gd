...
func run():
	...
	if "--nailong-preview" in OS.get_cmdline_user_args():
		...
		return
	check("all five archetypes land on physical ground",specimens.all(func(e): return e.is_on_floor() and absf(e.position.y)<.1))
	check("five silhouettes reuse shared mesh and material cache",Model.cache_snapshot().types.size()==5 and Model.cache_snapshot().materials<=10,Model.cache_snapshot())
	...
	var high = creature("roamer",Vector3(330,0,0),999)
	...
	check("invalid levels clamp to one and ten",low.level==1 and high.level==10)
