...
func run():
	...
	check("ACES keeps exposure fixed across profiles",env.tonemap_mode==Environment.TONE_MAPPER_ACES and env.tonemap_exposure==1)
	check("colour adjustment stays modest",is_equal_approx(env.adjustment_contrast,1.03) and is_equal_approx(env.adjustment_saturation,1.05))
