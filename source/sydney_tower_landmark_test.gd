...
func run()->void:
	...
	verify(w.get_meta("sydney_tower_geometry").windows==420,"turret has 420 distinct window panels")
...
func capture(w:Node3D)->void:
	...
	var folder:=ProjectSettings.globalize_path("res://../reports/sydney-tower-v013");DirAccess.make_dir_recursive_absolute(folder)
