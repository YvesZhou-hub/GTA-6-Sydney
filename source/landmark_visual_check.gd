...
func run():
	...
	var release_capture: bool=release_mode or final_mode
	var manual_render: bool=performance_mode or release_capture
	...
	var folder=ProjectSettings.globalize_path("res://../reports/release-final" if final_mode else "res://../reports/release-review" if release_mode else "res://../reports/landmark-review")
	...
	for shot in shots:
		...
		var saved:bool=root.get_texture().get_image().save_png(folder+"/"+shot[0]+".png")==OK
		if not saved:failures+=1
		var frame: Dictionary={"view":shot[0],"captured_utc":Time.get_datetime_string_from_system(true),"camera":[shot[1].x,shot[1].y,shot[1].z],"target":[shot[2].x,shot[2].y,shot[2].z],"ready":true,"saved":saved,"startup_ms":startup_ms,"draw_calls":Performance.get_monitor(Performance.RENDER_TOTAL_DRAW_CALLS_IN_FRAME)}
		if performance_mode or (release_mode and shot[0] in ["city_overview","george_street","darling_square"]) or (final_mode and shot[0]=="city_overview"):
	...
	print("LANDMARK_CAPTURE_COMPLETE frames=",evidence.size()," failures=",failures)
	game.active=false
