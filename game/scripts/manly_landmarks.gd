...
static func metadata() -> Array[Dictionary]:
	return [
		{"id":"manly_wharf","name":"Manly Wharf","center":WHARF_CENTER,"osm_ids":[7981564],"height_m":13.2,"height_confidence":"estimated from inspected exterior photograph","footprint_confidence":"OSM mapped outline","source":"https://apps.environment.nsw.gov.au/dpcheritageapp/ViewHeritageItemDetails.aspx?ID=5051365"},
		{"id":"the_corso","name":"The Corso","center":CORSO_CENTER,"osm_ids":[223785178,546705240],"height_m":0.0,"height_confidence":"game ground datum","footprint_confidence":"OSM pedestrian polygon and route; tree spacing is a reconstruction","source":"https://www.manlyaustralia.com.au/info/thingstodo/walks/manly-heritage-plaques/"},
		{"id":"hotel_steyne","name":"Hotel Steyne","center":HOTEL_CENTER,"osm_ids":[7889573,223786812,552008767],"height_m":11.4,"height_confidence":"estimated two-storey Art Deco exterior with roof parapet","footprint_confidence":"OSM multipolygon with true courtyard hole","source":"https://hotelsteyne.com.au/"}
	]
...
static func build(world: Node3D) -> void:
	...
	_beach_pines(world)
	_bus_stops(world)
	world.set_meta("manly_landmarks",metadata())
	world.anchors["manly_wharf"] = geo(-33.79933,151.28420)
	world.anchors["manly_corso"] = geo(-33.7977726,151.2870193)
	world.anchors["manly_beach"] = geo(-33.7970494,151.2883603)
	world.anchors["hotel_steyne"] = geo(-33.79712,151.28796)

static func _materials(world: Node3D) -> void:
	for entry: Array in [["manly_white","ede9d9"],["manly_roof","677972"],["manly_brick","a47552"],["manly_frame","b8c8ac"],["manly_glass","334d4c"],["manly_paving","c9bc97"],["manly_paving_band","807f72"],["manly_pine","344a37"],["manly_palm","657645"],["manly_palm_old","7c7353"],["manly_bus","237b9e"],["manly_ferry","33845d"]]:
...
static func _wharf(world: Node3D) -> void:
	...
	world._structure_mesh("manly/wharf/platform",prism(WHARF_POINTS,-1.0,0.0),WHARF_CENTER,"concrete",1200000)
	...
	for y in [2.9,3.45,4.0]:
		_local_box(world,tower,Vector3(0,y,0),Vector3(5.25,0.23,4.6),"manly_roof")
	for face in [-1,1]:
		...
		dial.mesh=dial_mesh; dial.material_override=world.materials.manly_roof
		...
		for mark in range(12):
			...
			_local_box(world,tower,Vector3(sin(angle)*0.84,4.8+cos(angle)*0.84,face*2.14),Vector3(0.07,0.15,0.07),"manly_white",Basis(Vector3.BACK,-angle))
		world._local_beam(tower,Vector3(0,4.8,face*2.17),Vector3(0,5.46,face*2.17),0.065,"manly_white")
		world._local_beam(tower,Vector3(0,4.8,face*2.17),Vector3(0.45,4.63,face*2.17),0.08,"manly_white")
	...
	var label_p := geo(-33.799550,151.284167,9.15)
	_label(world,"MANLY WHARF",label_p,0.026,Basis(Vector3.UP,deg_to_rad(-35)),Color("344b41"))
...
static func _corso(world: Node3D) -> void:
	...
	for station in range(28,int(a.distance_to(b))-18,27):
		for side in [-1,1]:
			var p: Vector3 = a+direction*station+right*side*5.6
			# Seat positions are inferred from the photo; trees use mapped points below.
			_ring_seat(world,p)
...
static func _palm(world: Node3D, p: Vector3, height: float) -> void:
	...
	for frond in range(12):
		var angle := frond*TAU/12.0
		var radial := Vector3(sin(angle),0,cos(angle))
		var lateral := Vector3(radial.z,0,-radial.x)
		var start := Vector3.UP*height
		var shoulder := start+radial*1.6+Vector3.UP*0.7
		var tip := start+radial*3.8-Vector3.UP*1.15
		# Each frond has a narrow stem and its own split fan, leaving real sky gaps.
		world._beam(p+start,p+shoulder,0.055,"manly_palm")
		for fan in range(7):
			var offset := (fan-3)*0.26
			_triangle(st,shoulder+lateral*offset*0.24,tip+lateral*offset,tip+lateral*(offset+0.15)+Vector3.UP*0.25,Vector3.UP)
		var old_top := start-radial*0.05-Vector3.UP*0.3
		var old_bottom := start+radial*0.9-Vector3.UP*2.0
		_triangle(st,old_top-lateral*0.2,old_top+lateral*0.2,old_bottom,radial)
	var view := MeshInstance3D.new()
	view.mesh=_finish(st)
	view.material_override=world.materials.manly_palm
	view.position=p
	view.visibility_range_end=700
	world.add_child(view)

static func _hotel(world: Node3D) -> void:
	# The surveyed multipolygon courtyard stays open through both storeys and
	...
	for level in range(2):
		...
		for i in range(HOTEL_POINTS.size()):
			...
			if level==0 and i in [1,2,3,4]:
				_local_box(world,body,mid+Vector3.UP*3.9+basis.z*1.25,Vector3(length+0.05,0.20,2.8),"manly_frame",basis)
				for stripe in range(int(length/0.8)):
					var p := mid+basis.x*(stripe*0.8-length*0.5+0.4)+Vector3.UP*4.03+basis.z*1.25
					_local_box(world,body,p,Vector3(0.37,0.065,2.8),"manly_white",basis)
	...
	_label(roof,"HOTEL STEYNE",sign_p+basis.z*0.32,0.020,basis,Color("e9debb"))
	_label(roof,"1859",sign_p+Vector3.DOWN*0.64+basis.z*0.34,0.011,basis,Color("e9debb"))
