...
SOURCE_FILES = ('world_osm_reference.json', 'world_osm_water.json', 'world_geography.json',
                ...
                'combat_crush_test.gd', 'combat_crush_envelope_test.gd', 'combat_weapons_test.gd', 'combat_spawn_test.gd',
                'runtime_diagnostics_test.gd', 'material_roles_test.gd', 'facade_night_test.gd', 'facade_stream_test.gd',
                'render_environment_test.gd', 'daylight_cycle_test.gd', 'city_clock_test.gd', 'vehicle_factory_cache_test.gd', 'public_lighting_test.gd', 'fixtures/save_store_v012.gd')
    ...
    for folder in ('game', 'licenses', 'docs'):
        for path in (root / folder).rglob('*'):
            ...
            if any(part in ('.godot', '__pycache__') for part in path.parts):
                continue
            if path.name == '.DS_Store' or path.suffix.lower() not in ('.md', '.txt', '.json', '.png', '.hdr', '.svg', '.gd', '.gdshader', '.uid', '.tscn', '.cfg', '.godot', '.import'):
                continue
            files.append(path)
