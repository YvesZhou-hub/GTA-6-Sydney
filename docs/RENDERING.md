...
整片陆地原来统一是草绿色，市中心看起来像草坪。现在启动时用**地图里的建筑轮廓**生成一张 512×512 的“城市化”遮罩（缩小再放大做模糊，边界带噪声），地表着色器据此在草地和铺装之间过渡：楼多的地方是铺装灰，空地保持草地，公园和沙滩仍然画在上层，保持绿色与沙色。遮罩范围与建筑数量记录在世界的 `urban_mask_area` 元数据里。

公园里另外撒了灌木、蕨类和草丛：按公园多边形自身的位置取点（同一个公园每次一样），只落在多边形内部，全城上限 2,200 株。

## 夜晚：路灯会亮

- 每盏路灯顶部有一个自发光的小灯头，随昼夜曲线从 0.05 亮到 3.2，白天几乎看不出来，天黑后整条街都亮起来。
- 真实光源只有 **10 盏**，在玩家 95 米内跟随最近的灯头移动，夜里才打开。全城约 1,960 盏灯不可能每盏都做成实时光源，这样既有地面光斑又不增加成本。
- 建筑窗户和临街店铺的夜间亮灯沿用 v0.3.0 的立面着色器。

验证：`--street-qa` 检查夜里灯亮、白天灯灭，以及各项街道行为；`--daylight-qa` 保持原有昼夜检查。

# 晴日光照、水面与画质档位

## 2026-09 画面升级：光照、立面与自然表面

**色调与环境光。** 色调映射从 ACES 改为 AgX：ACES 会把日照下的人行道和地平线天空压成纯白，AgX 保留高光过渡与云层。原先环境光全部取自蓝色 HDR 天空，所有阴影和浅色表面都偏蓝；现在白天环境光 55% 来自天空、其余为暖中性色 `d9cdb8`（能量 0.55），夜间过渡到冷蓝 `8193bb`、天空占 85%。太阳能量系数 1.15 → 1.32，对比度 1.03 → 1.1，饱和度 1.05 → 1.16，标准与精细档 SSAO 0.9 → 1.15。曝光仍为日间 1.0、夜间 1.18。

**普通楼立面（`city_facade.gdshader`，全城约 15,000 栋共用，无新增几何或 draw call）。**
- 离地 4.2 m 内按 OSM 用途区分：商业楼为玻璃橱窗（窗框、横档、暖色室内，亮度逐店不同）与招牌带（低饱和配色，部分带字条）；住宅为深色基座与门；工业为卷帘门。
- 每栋楼有稳定的色调与冷暖偏移，同一风格的相邻街区不再像复制。
- 楼层横带、窗台高光、墙脚污渍与雨痕；玻璃幕墙的层间为深色拱肩板。
- 近处窗户逐格变化：明暗不同的房间、部分百叶，砌体窗洞边缘有阴影；玻璃在掠射角反射更多天空。
- 屋顶在防水膜、砾石与混凝土之间变化，带表面噪声。
- 夜间多数商铺亮起暖光，招牌带微光；原有夜间亮窗函数一字未改。
- 细节全部随屏幕导数淡出，远景不闪烁。

**树、草坪与路面（`harbor_world.gd`）。** 树冠球体分段 12×6 → 18×9，加世界坐标三平面叶团纹理、凹凸法线贴图与背光透射；草坪加大尺度干湿斑块与细颗粒；路面加沥青骨料与补丁纹理，颜色从偏蓝绿的 `3f494b` 改为中性 `5b5c5f`，镜面反射降低以免掠射角反射蓝天。纹理在启动时由 `FastNoiseLite` 生成（256²），材质仍为 `StandardMaterial3D`，材质角色与诊断调参照常生效。

**性能。** Apple M4 / Metal、1440×900、标准画质静态机位：城市俯瞰 16.3 ms（改前 17.3）、George Street 11.7 ms（11.5）、Darling Square 6.95 ms（6.9），draw call 不变；启动增加约 1 秒用于生成噪声纹理。

**已知限制。** 远景夜间 CBD 仍偏亮偏白（改前即如此）；树仍是三团球形树冠，没有真实枝叶几何；街道仍缺少车流与行人。
